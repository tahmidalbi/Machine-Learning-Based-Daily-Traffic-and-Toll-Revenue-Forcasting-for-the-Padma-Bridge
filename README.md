...
| Folder | What it does |
|---|---|
| [`padma_bridge_ml(feature-engineered version)/`](./padma_bridge_ml(feature-engineered%20version)) | An end-to-end, reproducible pipeline that forecasts **next-day** total traffic and toll revenue and runs the full econometric impact study (weather/holiday/Eid effects, Padma–Jamuna Granger analysis, railway intervention analysis, nighttime-light difference-in-differences). Self-contained with raw CSV inputs and a Colab notebook. 
